...
% for i=1:size(part,1)
%     span(i)=part(i,2)-part(i,1)+1-sum(ismember(pixelCount(part(i,1):part(i,2)),0));
% end
...
subplot(2,2,4);
imhist(data);
title('Histogram of DHE image');

% % Get size of existing image A. 
% [rowsA1 colsA1 numberOfColorChannelsA1] = size(z); 
% % Get size of existing image B. 
% [rowsB1 colsB1 numberOfColorChannelsB1] = size(data); 
% % See if lateral sizes match. 
% if rowsB1 ~= rowsA1 || colsA1 ~= colsB1 
% % Size of B does not match A, so resize B to match A's size. 
% data = imresize(data, [rowsA1 colsA1]);
end
